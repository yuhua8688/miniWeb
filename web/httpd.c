			...
			else
			{
				...
				switch(*ptr){
				 ...
				 case 'i':
				 {
					...
					rt_memset(i_buff, 0, tmp + 1);//加一位存结束符
					if (*ptr == '/')
					{
	...
	{	
		...
		else if (*param == 'm' || *param == 'M')	//type:multipart/form-data
		{
			...
			*param = 0;
			...
			param += 4;
			p_value = param;
			stream_size = stream_size - (p_value - p_start);//计算剩余字节
			stream_size = stream_size - boundary_len - 8;//计算文件长度
			len = len - (p_value - data);//计算这个buff剩余长度
			p_start = param;
